...
public class CubeDefinitionMixin implements ICubeDefinition {
    ...
    @Override
    public BendableCuboid bakeBendableCuboid(int texWidth, int texHeight) {
        return new BendableCuboid((int)this.texCoord.u(), (int)this.texCoord.v(), this.origin.x(), this.origin.y(), this.origin.z(), this.dimensions.x(), this.dimensions.y(), this.dimensions.z(), this.grow.growX, this.grow.growY, this.grow.growZ, this.mirror, (float)texWidth * this.texScale.u(), (float)texHeight * this.texScale.v(), this.visibleFaces);
    }
}
